...
param(
    ...
    [string]$PropertyPackageName = "Chao-Seader",
    [string]$Model = "Chao-Seader",
    [string]$PropertyPackageSourceFile = "ChaoSeader.vb",
    [string]$ModelSourceFile = "ChaoSeader.vb"
)
...
$packageSource=Join-Path $source ("DWSIM.Thermodynamics\PropertyPackages\"+$PropertyPackageSourceFile)
$modelSource=Join-Path $source ("DWSIM.Thermodynamics\PropertyPackages\Models\"+$ModelSourceFile)
...
$pp=[DwsimChaoSeaderExtractor]::Package($fs,$PropertyPackageName)
...
$doc=[ordered]@{schema_version="dwsim-semi-empirical-data-1";model=$Model;source=[ordered]@{product="DWSIM";revision=$DwsimRevision;case_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $case).Hash.ToLowerInvariant();runtime_assembly_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $assembly).Hash.ToLowerInvariant();property_package_class=[DwsimChaoSeaderExtractor]::TypeName($pp);property_package_source=("DWSIM.Thermodynamics/PropertyPackages/"+$PropertyPackageSourceFile);property_package_source_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $packageSource).Hash.ToLowerInvariant();model_source=("DWSIM.Thermodynamics/PropertyPackages/Models/"+$ModelSourceFile);model_source_sha256=(Get-FileHash -Algorithm SHA256 -LiteralPath $modelSource).Hash.ToLowerInvariant()};compounds=$records}

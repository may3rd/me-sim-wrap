...
The frozen compound catalog and all three pure-property datasets cover the same 14
case-distinct DWSIM names. The PR system exposes those records through `ideal`,
`saturated_liquid`, and `transport`. This is property coverage, not a claim that all
91 possible binary combinations are supported: `pr-v1.json` contains 42 explicit pairs,

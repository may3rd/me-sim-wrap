...
Lee-Kesler-Plöcker data is regenerated by `scripts/extract_dwsim_lkp_data.ps1`.
The model implements the source's Plöcker critical mixing, simple/heavy reference-fluid
roots, residual enthalpy term, and composition derivatives. Its phase-fugacity vectors
match the repeat capture within `1e-9` absolute; its tightly converged split is compared
to DWSIM's saved five-iteration flash with `1e-4` absolute and relative tolerances.

Steam Tables data is regenerated by `scripts/extract_dwsim_steam_tables_data.ps1`.
The package is absent from DWSIM's automation factory, so the golden explicitly records
direct-class construction over a saved pure-Water compound domain. DWSIM uses IAPWS
region 4 for its pure flash but its inherited `AUX_PVAPi` Water correlation for liquid
fugacity; the system preserves and independently tests both paths. Caloric and transport
IF97 regions remain outside this first boundary.

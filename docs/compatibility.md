...
`tests/golden/u3-pipe-liquid-pr-eos.json` covers a five-increment, pure n-pentane PR pipe at 300 K. The kernel reproduces DWSIM's friction and static pressure-drop totals from each captured liquid segment state within `1e-5` relative, using its source friction-factor regimes: laminar below Re 2100, Churchill transition, and the explicit turbulent correlation above Re 4000. It provides DWSIM's fixed-K fitting equation, its ISO-5167-style incompressible orifice calculation with corner, flange, and radius taps, its homogeneous Lockhart-Martinelli multiplier, its legacy-unit Beggs-Brill inclined-flow calculation when phase properties are supplied, and the DWSIM API RP 520 vapor- and liquid-sizing utility equations. `tests/golden/u3-psv-vapor-api520.json` records the methane PSV state and saved utility inputs; its confirmed DWSIM window result is `0.08013877 in²`, selecting `D / 0.11 in²`. `tests/golden/u3-psv-liquid-api520.json` records the liquid n-pentane state; the source equation returns `0.00480318 in²`, also selecting D. The DWSIM utility does not expose a standard edition, so this is source-equation parity only, not relief-design or safety acceptance. Thermal coupling, compressible flow, phase-property calculation, fitting catalogue data, orifice and two-phase golden-case parity, two-phase relief, and full pipe-operation parity remain unsupported.

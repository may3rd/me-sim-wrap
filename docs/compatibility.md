...
`tests/golden/u3-pipe-thermal-liquid-pr-eos.json` and its normalized repeat are DWSIM 9.0.5 references for defined-HTC liquid-pipe thermal coupling. The captured feed is 95.2381 mol% n-pentane and 4.7619 mol% ethane at 300 K, 500 kPa, and 200 mol/s; the 100 m carbon-steel pipe has five saved increments, U = 25 W/m2/K, and a constant 350 K external temperature. `pipe_defined_htc_heat_transfer` uses the exact constant-property exponential form equivalent to the DWSIM LMTD equation when liquid heat capacity is supplied. `pipe_defined_htc_profile` advances that primitive across supplied segment lengths and heat capacities, returning the segment results, outlet temperature, accumulated area, and total heat. `liquid_pipe_supplied_state_profile` combines it with the supplied-state pressure profile, enforces one mass-flow basis, and closes the captured outlet pressure, outlet temperature, and energy-stream magnitude in one result. The saved structure contains five hydraulic increments but advances the active thermal profile from result row 2 through row 5; the coupled gate preserves those distinct authoritative counts. Segment duties match within `3e-3` relative, outlet pressure within `1e-5`, outlet temperature within `1e-4`, and energy within `2e-3`. `pipe_defined_htc_gradient_profile` follows the vendored DWSIM source rule: evaluate the ambient temperature from the segment-start cumulative distance and hold it constant through that increment. Its zero-gradient reduction passes the existing golden, but nonzero-gradient parity remains unverified pending a dedicated DWSIM capture. Liquid heat-capacity and viscosity calculation, estimated-HTC modes, insulation, radiation, phase change, and iterative pressure/thermal property updates remain unsupported.

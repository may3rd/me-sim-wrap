...
The optional `-CaptureColumnProfiles` reference-capture switch preserves DWSIM's full-precision public `Tf`, `Lf`, `Vf`, `xf`, `yf`, and `Kf` arrays; it is opt-in so existing golden schemas stay stable. `fixed_k_material_column` uses the captured `Kf` only as a frozen thermodynamic closure and independently solves every stage component balance plus vapor summation with positive log flows, reduced softmax liquid compositions, finite-difference Newton steps, and a residual-reducing line search. The reboiled-absorber gate reaches a scaled residual below `1.4e-15`; liquid and vapor compositions match within `4e-14`, while internal flows match within `2.2e-7 kmol/s`, consistent with correction of DWSIM's saved `0.001` column tolerance. Singular Jacobians, failed line searches, and iteration exhaustion return no partial profile and preserve immutable accepted-iteration history. Temperature, enthalpy, pressure-profile, duty/specification, and live equilibrium-ratio iteration remain unsupported, so this is fixed-K material parity rather than a fully energy-coupled rigorous-column claim.

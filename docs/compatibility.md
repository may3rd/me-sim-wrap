...
`tests/golden/u3-pipe-thermal-estimated-htc-dry-soil-liquid-pr-eos.json` and its normalized repeat cover DWSIM's saved dry-soil medium at 310 K and 1 m initial burial depth. The saved catalog selection maps to 0.5 W/m/K soil conductivity. `pipe_estimated_htc_soil` reproduces DWSIM's buried-cylinder resistance and its 2.4614250971428842 W/m2/K external contribution within `1e-12`; internal and wall coefficients retain their established `1e-12` and `5e-6` gates, and overall U matches within `1e-7`. `pipe_estimated_htc_soil_pr_profile` uses the shared pressure-endpoint PR enthalpy balance and matches segment duties within `3e-3`, outlet temperature within `1e-4`, and aggregate energy within `2e-3` or 35 W absolute. Burial depth and soil conductivity are explicit inputs; the remaining DWSIM terrain catalog selections have not yet been captured.

`tests/golden/u3-pipe-thermal-estimated-htc-insulated-irradiated-liquid-pr-eos.json` and its normalized repeat enable local irradiation at 0.01 kWh/m2 with absorption efficiency 0.1. `pipe_absorbed_solar_radiation` follows DWSIM's steady source equation, including its use of the bare outer diameter for both residence volume and exposed area, yielding 2.789834 kW absorbed per active segment. `pipe_irradiated_heat_transfer` solves the coupled LMTD and radiation balance; the profile retains `3e-3` segment-duty, `1e-4` outlet-temperature, and 35 W aggregate-energy gates. This scope requires supplied liquid heat capacity, conductivity, viscosity, density, velocity, and inlet volumetric flow. Calculation of those properties, global-weather irradiation, uncaptured terrain catalog mappings, phase change, and iterative transport-property updates remain unsupported.

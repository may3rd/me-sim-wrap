"""DWSIM Modified UNIFAC (Dortmund) data and activity equation."""
...
@dataclass(frozen=True, slots=True)
class ModfacData:
    source_revision: str; groups_sha256: str; interactions_sha256: str
    ...
    def compound(self, compound_id: str) -> ModfacCompoundBasis:
        ...
        raise MissingCompoundData(f"missing Dortmund compound basis: {compound_id}")
    def group(self, secondary_id: int) -> ModfacGroup:
        for record in self.groups:
            if record.secondary_id == secondary_id: return record
        raise ValidationError(f"missing Dortmund subgroup: {secondary_id}")
...
def load_modfac_data(path: str | Path) -> ModfacData:
    try:
        ...
        if doc["schema_version"]!="dwsim-modfac-data-1" or doc["model"]!="Modified UNIFAC (Dortmund)":
            raise ValidationError("unsupported Dortmund data schema or model")
        s=doc["source"]
        ...
        data=ModfacData(s["revision"],s["groups_sha256"],s["interactions_sha256"],compounds,groups,pairs)
    except ValidationError: raise
    except (KeyError,TypeError,ValueError) as error: raise ValidationError(f"invalid Dortmund data: {error}") from error
    if not data.source_revision or len(data.groups_sha256)!=64 or len(data.interactions_sha256)!=64 or len(data.compound_basis)!=2 or len(data.groups)!=108 or len(data.interaction_pairs)!=1167:
        raise ValidationError("Dortmund source identity or record count is invalid")
    if len({x.compound_id for x in data.compound_basis})!=2 or len({x.secondary_id for x in data.groups})!=len(data.groups) or len({(x.first_primary_id,x.second_primary_id) for x in data.interaction_pairs})!=len(data.interaction_pairs):
        raise ValidationError("Dortmund keys must be unique")
    for g in data.groups:
        if g.primary_id<=0 or g.secondary_id<=0 or not g.primary_name or not g.group_name or not math.isfinite(g.r) or g.r<=0 or not math.isfinite(g.q) or g.q<0: raise ValidationError("invalid Dortmund group")
    for p in data.interaction_pairs:
        if p.first_primary_id<=0 or p.second_primary_id<=0 or p.first_primary_id==p.second_primary_id or any(not math.isfinite(v) for v in (p.a12,p.b12,p.c12,p.a21,p.b21,p.c21)): raise ValidationError("invalid Dortmund interaction")
    for c in data.compound_basis:
        if not c.compound_id or c.q<=0 or c.r<=0 or not c.group_surface_fractions or not math.isclose(math.fsum(v.value for v in c.group_surface_fractions),1.0,abs_tol=1e-12): raise ValidationError("invalid Dortmund compound basis")
        for v in c.group_surface_fractions:
            if v.value<=0 or not math.isfinite(v.value): raise ValidationError("invalid Dortmund surface fraction")
...
def modfac_activity_coefficients(data: ModfacData, compound_ids, composition, temperature_k: float) -> tuple[float,...]:
    ...
    try:
        ...
        for i,c in enumerate(basis):
            ...
            if not math.isfinite(gamma) or gamma<=0: raise ValidationError("Dortmund activity coefficient is unrepresentable")
            out.append(gamma)
    except (OverflowError,ValueError,ZeroDivisionError) as error: raise ValidationError("Dortmund state is outside the representable range") from error
    return tuple(out)

        ...
        Public Overrides Function GetPropertyValue(ByVal prop As String, Optional ByVal su As Interfaces.IUnitsOfMeasure = Nothing) As Object
            ...
            If val0 Is Nothing Then
                ...
                ElseIf prop.StartsWith("Activity Coefficient") Then
                    ...
                    If prop.Contains("Vapor Phase") Then
                        Return Phases(2).Compounds(comp).ActivityCoeff.GetValueOrDefault()
                    ElseIf prop.Contains("Liquid Phase 1") Then
                        Return Phases(3).Compounds(comp).ActivityCoeff.GetValueOrDefault()
                    Else
                        Return Phases(4).Compounds(comp).ActivityCoeff.GetValueOrDefault()
                    End If

                ElseIf prop.StartsWith("Fugacity Coefficient") Then

                    Dim comp = prop.Split("/")(1).Trim()
                    If Not Phases(0).Compounds.ContainsKey(comp) Then Return 0.0

                    If prop.Contains("Vapor Phase") Then
                        Return Phases(2).Compounds(comp).FugacityCoeff.GetValueOrDefault().ConvertFromSI(su.pressure)
                    ElseIf prop.Contains("Liquid Phase 1") Then
                        Return Phases(3).Compounds(comp).FugacityCoeff.GetValueOrDefault().ConvertFromSI(su.pressure)
                    Else
                        Return Phases(4).Compounds(comp).FugacityCoeff.GetValueOrDefault().ConvertFromSI(su.pressure)
                    End If

                ElseIf prop.StartsWith("Diffusion Coefficient") Then

                    Dim comp = prop.Split("/")(1).Trim()
                    If Not Phases(0).Compounds.ContainsKey(comp) Then Return 0.0

                    If prop.Contains("Vapor Phase") Then
                        Return Phases(2).Compounds(comp).DiffusionCoefficient.GetValueOrDefault().ConvertFromSI(su.diffusivity)
                    ElseIf prop.Contains("Liquid Phase 1") Then
                        Return Phases(3).Compounds(comp).DiffusionCoefficient.GetValueOrDefault().ConvertFromSI(su.diffusivity)
                    Else
                        Return Phases(4).Compounds(comp).DiffusionCoefficient.GetValueOrDefault().ConvertFromSI(su.diffusivity)
                    End If

                ElseIf prop.StartsWith("PROP_MS") Then
            ...
            Select Case phase
                ...
                Case PhaseLabel.Mixture
                    ...
                    For Each sub1 In Me.Phases(0).Compounds.Values
                        ...
                        sub1.MolarFlow = M * sub1.MoleFraction
                        sub1.MassFlow = W * sub1.MassFraction
                    Next
